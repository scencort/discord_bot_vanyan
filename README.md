...
- anti-raid/anti-nuke
- AutoMod (спам, капс, ссылки, слова)
...
4. Пригласи бота с правами:
   ...
   - `Mute Members`
   - `Deafen Members`
   - `Manage Roles` (желательно для anti-nuke)
   - `View Audit Log` (для anti-nuke)
   - `Connect`
   - `Speak` (опционально)
...
- `/set_raid_limits join_per_minute:<n> channel_create_per_minute:<n> role_create_per_minute:<n> mention_limit:<n>`
- `/set_automod_words words:<слово1,слово2,...>`
- `/set_automod_whitelist domains:<site.com,domain.net,...>`
- `/set_automod_exempt_channel channel:<#канал> mode:add|remove`
- `/set_automod_exempt_role role:<@роль> mode:add|remove`
...
Поддерживаемая авто-отмена: `ban`, `timeout`, `voice_ban`, `voice_mute`, `voice_deafen`, а также auto-наказания от warn/automod.
...
## 9. Anti-Raid / Anti-Nuke

Включено событиями:

- всплеск входов (`on_member_join`)
- всплеск создания каналов (`on_guild_channel_create`)
- всплеск создания ролей (`on_guild_role_create`)
- массовые mention в сообщениях

Поведение: авто-блок (timeout), очистка созданных объектов при ньюке и алерт в alert-channel.

## 10. AutoMod

Проверяет:

- spam (много сообщений за короткий интервал)
- caps flood
- ссылки вне whitelist
- запрещенные слова

Эскалация по числу нарушений пользователя:
...
- Для anti-nuke обязательно дать боту `View Audit Log`, иначе определение нарушителя будет ограничено.
- Для корректной работы AutoMod нужен `MESSAGE CONTENT INTENT`.
